...
try:
    ...
    if os.path.exists(nltk_data_dir) and nltk_data_dir not in nltk.data.path:
        ...
        import logging
        logging.getLogger().debug(f"已添加 NLTK 数据路径: {nltk_data_dir}")
except:
    pass  # 如果导入失败，忽略
...
def get_tts_wav(
    ...
    # 记录参考音频和文本信息
    logger.info(f"[get_tts_wav] 参考音频路径: {ref_wav_path}")
    logger.info(f"[get_tts_wav] 参考文本: {prompt_text}")
    logger.info(f"[get_tts_wav] 参考语言: {prompt_language}")
    logger.info(f"[get_tts_wav] 目标文本: {text[:100]}..." if len(text) > 100 else f"[get_tts_wav] 目标文本: {text}")
    logger.info(f"[get_tts_wav] 目标语言: {text_language}")
    
    dtype = torch.float16 if is_half == True else torch.float32
    zero_wav = np.zeros(int(hps.data.sampling_rate * 0.3), dtype=np.float16 if is_half == True else np.float32)
    with torch.no_grad():
        logger.info(f"[get_tts_wav] 开始加载参考音频: {ref_wav_path}")
        wav16k, sr = librosa.load(ref_wav_path, sr=16000)
        logger.info(f"[get_tts_wav] 参考音频加载完成，采样率: {sr}, 长度: {len(wav16k)/sr:.2f}秒")
    ...
    def _split_long_texts(items, max_len=120):
        result = []
        for _t in items:
            t = _t.strip()
            if not t:
    ...
    texts = _split_long_texts(texts, max_len=120)
    audio_bytes = BytesIO()
